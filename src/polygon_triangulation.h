...
public:
    ...
    // Mirror engine-module API: fill flat Godot Vector<int> with
    // `out_tri_count * 3` vertex indices that refer back into the
    // input point array. Backs everything the godot-cpp consumers
    // need without leaking Eigen into the public surface.
    void get_indexed_result(Vector<int> &out_face_indices,
            int &out_tri_count, int &out_point_count) const;

    // Internal accessor — used by CassieTriangulator to feed the
    // result straight into pmp's refine_patch (Eigen-typed). Not
    // bound to GDScript.
