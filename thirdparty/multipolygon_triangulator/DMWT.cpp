...
#include "../../src/delaunay_geogram.h"
...
void DMWT::genTriCandidates() {
  ...
  if (coplanar) {
    // Project to (x, y) plane and run Geogram BDEL2d.
    std::vector<double> xy;
    xy.reserve(std::size_t(numofpoints) * 2u);
    for (int i = 0; i < numofpoints; ++i) {
      xy.push_back(points[i * 3 + 0]);
      xy.push_back(points[i * 3 + 1]);
    }
